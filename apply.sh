...
original_author="Carmo-sousa"
original_name="blog_flask"
original_urlname="blog-flask"
original_description="Awesome blog_flask created by Carmo-sousa" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/blog_flask "${name}"
...
then
    ...
    echo "then run 'blog_flask' or 'python -m blog_flask'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'blog_flask' or 'python -m blog_flask' or 'poetry run blog_flask'"
fi
