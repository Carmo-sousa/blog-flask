...
- 🎯 Entry points to execute your program using `python -m <blog_flask>` or `$ blog_flask` with basic CLI argument parsing.
...
---
# blog_flask Flask Application

Awesome blog_flask created by Carmo-sousa
...
```bash
git clone https://github.com/Carmo-sousa/blog-flask blog_flask
cd blog_flask
make install
```
...
```bash
blog_flask create-db   # run once
blog_flask populate-db  # run once (optional)
blog_flask add-user -u admin -p 1234  # ads a user
blog_flask run
```
